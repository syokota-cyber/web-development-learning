# 🔖 バージョン管理ルール（2025年8月22日追加）

## 「一元管理・自動リマインド・段階的改善」原則

### ⚠️ **重要: 機能追加・修正時の必須確認**
**AIは以下を必ず実行すること:**
1. 機能追加・修正の開始時に「バージョン更新が必要ですか？」と確認
2. 必要な場合は `VERSION_UPDATE_CHECKLIST.md` の手順を提示
3. `src/config/version.js` が唯一の更新場所であることを強調

### 📍 **バージョン情報の管理場所**
```
src/config/version.js  ← 唯一の真実（Single Source of Truth）
├── CURRENT_VERSION    ← 現在のバージョン番号
├── RELEASE_DATE       ← リリース日
└── VERSION_HISTORY    ← 更新履歴（UpdateHistory.jsxが参照）
```

### 📅 **フェーズ移行スケジュール**
- **現在: Phase 1** - 手動更新（version.js のみ更新）
- **2025年8月29日: Phase 2** - Git タグ運用開始
- **2025年9月5日: Phase 3** - 半自動化（GitHub Actions リマインダー）
- **2025年9月19日: Phase 4** - 完全自動化

### 🔔 **AIへのリマインダー指示**
1. **デプロイ前**: 「version.js は更新しましたか？」と確認
2. **不整合検知時**: 即座に警告を出す
3. **フェーズ移行日**: 1週間前と当日にリマインド
4. **定期確認**: 週1回バージョン管理状況をチェック

---

# 📁 プロジェクト管理・運用ルール（2025年8月21日追加）

## 「フォルダ明確化・作業場所統一・同期自動化」原則

### フォルダ構成と役割分担
```
📁 camping-car-apps/ (プロジェクトルート)
├── 📁 travel-journal/                 ← **メイン開発フォルダ (PRIMARY)**
│   ├── .vercel/                       ← 本番デプロイ設定
│   ├── src/                          ← **全ての開発作業はここで実施**
│   ├── .env                          ← 本番環境変数
│   └── [完全なReactアプリ構成]
├── 📁 travel-journal-backup-YYYYMMDD/ ← **バックアップフォルダ (BACKUP)**
│   └── [緊急時復旧用・参照専用]
├── 📁 docs/                          ← **ドキュメント類**
├── 📁 learning-logs/                  ← **学習記録**
├── 📁 supabase/                       ← **DB Migration管理**
└── PROJECT_DIRECTORY_MAP.md           ← **ディレクトリマップ（必読）**
```
...
```bash
# Step 1: メインフォルダで作業開始
cd /path/to/camping-car-apps/travel-journal
...
| 作業内容 | 使用フォルダ | 理由 |
|---------|-------------|------|
| 新機能開発・バグ修正 | `travel-journal/` | 本番デプロイ直結 |
| 緊急復旧 | `travel-journal-backup-*/` → コピー | バックアップから復元 |
| 参照・確認 | どちらでも | 読み取り専用 |

### 緊急時対応手順
1. **軽微な問題**: `git revert` でロールバック
2. **重大な障害**: バックアップから `travel-journal/` に復元
3. **Vercel設定問題**: `vercel link` で再接続
